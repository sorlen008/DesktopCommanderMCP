...
function parseDirectoryEntries(content: string): { hint: string; entries: DirEntry[] } {
    ...
    for (const line of entryLines) {
        ...
        flat.push({ name, fullPath: name, isDir, isDenied, depth: parts.length - 1 });
    }
    ...
    for (const item of flat) {
        ...
        const entry: DirEntry = { name: baseName, isDir: item.isDir, isDenied: item.isDenied, depth: item.depth, children: [], relativePath: item.fullPath };
